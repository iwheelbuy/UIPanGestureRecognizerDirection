...
extension DispatchQueue {
    ...
    class func once(file: String = #file, line: Int = #line, block: () -> Void) {
        let token = "\(file)+\(line)"
    ...
    private class func once(token: String, block: () -> Void) {
        ...
        guard array.contains(token) == false else { return }
        array.append(token)
        block()
    }
}
...
public extension UIPanGestureRecognizer {
    ...
    struct UIPanGestureRecognizerRuntimeKeys {
    ...
    public var direction: UIPanGestureRecognizer.Direction {
        get {
            ...
            return (object as? UIPanGestureRecognizer.Direction) ?? UIPanGestureRecognizer.Direction.all
        }
        set {
